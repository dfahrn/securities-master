...
One bar applies throughout: every non-obvious design decision must be
explainable and defensible, and the system must make its own correctness
...
Excluded: a REST API service. The research-facing artifacts are the point, and
a service layer adds nothing to them.

## 12. Build phases

Each phase ends with working software and a design note in `docs/notes/`.
