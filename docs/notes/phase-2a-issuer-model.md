...
This note explains the design of `core.issuer` and the rebuilt `core.security`
/ `core.security_identifier` / `core.security_listing`. It assumes the reader
has read `phase-1-identity.md` and knows what Phase 1 shipped. Each section
answers one question a reviewer could reasonably push back on, checked against
the code and the real re-seed run recorded below.
